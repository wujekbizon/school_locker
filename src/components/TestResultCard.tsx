...
export default async function TestResultCard({
  completedTest,
}: {
  ...
  const questionDetails = testsData?.map(({ testData, userCorrectAnswer }) => {
    ...
    return (
      <div
        key={id}
        className={`${correctAnswer?.option === userCorrectAnswer?.option ? "border-green-400" : "border-red-400"} flex w-full items-center justify-between rounded border p-2`}
      >
        <div className="w-2/3">
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4 overflow-y-auto rounded-xl border border-border/40 bg-black p-2 scrollbar-webkit md:p-8 lg:h-3/4 lg:w-3/4">
      <div>
